<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true" CodeBehind="AddCategory.aspx.cs" Inherits="ReferLocals.Admin.AddCategory" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        function EditCategory(btn) {
            ...
            $('.IconImage').append('<img src="<%=DataAccess.HelperClasses.Common.CategoryImagesPath%>' + image + '"/>');
            $("#divGridCategory").addClass("hide");
        ...
        function DisableCategory(btn) {
            var id = $(btn).attr('data-id');
            var Approved = $(btn).attr('data-isapprovedbyadmin');
            $.ajax({
                url: '/Admin/AddCategory.aspx/DisableCategory',
                type: "POST",
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="page-content" id="divGridCategory">
                            ...
                            <div class="row rowPadding">
                                <div class="col-md-6 col-sm-12">
                                    <div class="btn-group btn-group-solid">
                                        <a class="btn btn-lg red" style="cursor: pointer" onclick="return AddCategory();"><i class="fa fa-plus"></i>Add New Category </a>
                                        <a class="btn btn-lg green" type="button" href="/Admin/AddSubCategory"><i class="fa fa-link"></i>Add New Sub-Category </a>
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <asp:Repeater ID="rptCategories" runat="server">
                                                <ItemTemplate>
                                                    <tr data-name="<%#Eval("Name") %>">
                                                        <td><%#Eval("SNO") %> </td>
                                                        <td><a class="linktext" href="/Admin/AddSubCategory?Id=<%#Eval("Id") %>" ><%#Eval("Id") %> </a></td>
                                                        <td><a class="linktext" href="#">
                                                            <img src="<%=DataAccess.HelperClasses.Common.CategoryImagesPath %><%#Eval("Image") %>?width=50&height=50" alt="<%#Eval("Image") %>" >
                                                        </a></td>
                                                        <td><a class="linktext" href="/Admin/AddSubCategory?Id=<%#Eval("Id") %>" "><%#Eval("Name") %> </a></td>
                                                        ...
                                                        <td class="ancherwidth ancherwidth2"><a onclick="EditCategory(this);" data-id="<%#Eval("Id") %>" data-name="<%#Eval("Name") %>" data-description="<%#Eval("Description") %>" data-image="<%#Eval("Image") %>" data-isapprovedbyadmin="<%#Eval("IsApprovedByAdmin") %>" style="cursor: pointer" class="btn dark btn-sm btn-outline sbold uppercase"><i class="fa fa-edit"></i>Edit </a>
                                                            <a style='cursor: pointer;' class="btn <%#(Convert.ToBoolean(Eval("IsApprovedByAdmin"))==true?"dark":"red")%>   btn-sm btn-outline sbold uppercase" onclick="return DisableCategory(this);" data-id="<%#Eval("Id") %>" data-isapprovedbyadmin="<%#Eval("IsApprovedByAdmin") %>"><i class="fa fa-trash-o"></i><%#(Convert.ToBoolean(Eval("IsApprovedByAdmin"))==true?"Disable":"Enable")%> </a>
                                                        </td>
                                                    </tr>
                                                </ItemTemplate>
                                            </asp:Repeater>
    ...
    <div class="page-content hide" id="divAddCategory">
        <div class="page-bar">
            <ul class="page-breadcrumb">
        ...
        <div class="note note-success homesucess">
            <h3 style="margin-bottom: 0px;">Add New Category</h3>
        </div>
        <div class="row">
            <div class="col-md-12">
                <!-- BEGIN PORTLET-->
